                    ...
                    if (pasteHtml) {
                        ...
                        if (pasteHtml) {
                            ...
                            resultHtml = resultHtml.replace(/<p>(https?:\/\/.*?)<\/p>/ig, function (match, link) {
                                return '<p><a href="' + link + '" target="_blank">' + link + 'AD按时打算大神的</p>';
                            });
                        }
                    }
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        menu.clickEvent = function (e) {
            ...
            // 自定义command事件
            function commandFn() {
                $quote = $('<p><span class="diamond"></span>' + $rangeElem.text() + '</p>');
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if (link.indexOf('.mp4')==-1) {
                alert('插入的文件不是视频格式，请检查后重新插入!');
                menu.dropPanel.focusFirstInput();
                return;
            }
            
            var html = ' <div class="video J-Video"><span class="Play J-Play"></span><video src="'+link+'" >您的浏览器不支持 video 标签。</video><span class="close J-Close"></span></div>';
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if (link.indexOf('.mp3')==-1) {
                alert('插入的文件不是音频格式，请检查后重新插入!');
                menu.dropPanel.focusFirstInput();
                return;
            }
            
            var html = ' <div class="audio J-Audio"><span class="Play J-Play"></span><audio src="'+link+'" >您的浏览器不支持 video 标签。</audio><span class="close J-Close"></span></div>';
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        // 自定义UI，并添加到上传dom节点上
        var $uploadIcon = $('<input type="file" class="J-EditLoadImg" style="width: 100%; height: 100%; background-color: #000;" />');
        ...
        $(document).on('change', '.J-EditLoadImg', function(e){
        var file = $(this).get(0);
        console.log('J-EditLoadImg', file.files[0]);
        var formData = new FormData();
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if (data && data.code) {
                   var imgHtml = '<img style="max-width:100%;" src="' + data.url + '"/>';
                    editor.command(e, 'insertHtml', imgHtml, null); 
                }
